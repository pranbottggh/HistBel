...
GROQ_PROXY = os.getenv("GROQ_PROXY")  # новый параметр для прокси, например: "http://username:password@ip:port"
...
if GROQ_PROXY:
    ...
    client = Client(api_key=GROQ_API_KEY, client=httpx.Client(proxies=GROQ_PROXY, transport=transport))
else:
...
async def groq_chat(messages):
    try:
        ...
        # Блокируем длинный JSON и показываем пользователю дружелюбное сообщение
...
async def start(message: Message):
    ...
    await message.answer("🇧🇾 AI бот для изучения истории Беларуси", reply_markup=main_menu())
...
async def quiz_question(callback: CallbackQuery):
    ...
    prompt=f"""
Создай {level} вопрос по истории Беларуси.
